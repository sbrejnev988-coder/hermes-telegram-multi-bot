...
class TelegramAdapter(BasePlatformAdapter):
    ...
    @property
    def _telegram_env_platform(self) -> Platform:
        return getattr(self, "platform", Platform.TELEGRAM) or Platform.TELEGRAM

    def _telegram_env_names(self, suffix: str) -> tuple:
        from gateway.config import telegram_env_names
        return telegram_env_names(self._telegram_env_platform, suffix)

    def _telegram_env_value(self, suffix: str, default: str = "") -> str:
        for name in self._telegram_env_names(suffix):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform = Platform.TELEGRAM):
        ...
        # Bot API rich messages auto-detect URLs, mentions, commands, etc.;
        # operators can opt out per Telegram docs via rich_skip_entity_detection.
        self._rich_skip_entity_detection: bool = self._coerce_bool_extra(
            "rich_skip_entity_detection", False
        )
    ...
    def _should_attempt_rich_edit(
        self, content: str, metadata: Optional[Dict[str, Any]] = None
    ) -> bool:
        """True when final editMessageText should use Bot API rich_message.

        Streaming previews deliberately use plain edits. The final edit is the
        place where Telegram's official rich message formatting belongs:
        editMessageText supports a ``rich_message`` parameter with an
        InputRichMessage object (``markdown`` or ``html`` field), so final
        streamed replies do not need the duplicate fresh-message workaround.
        """
        return bool(
            getattr(self, "_rich_messages_enabled", False)
            and not getattr(self, "_rich_send_disabled", False)
    ...
    def _rich_message_payload(
        ...
        if skip_entity_detection or getattr(self, "_rich_skip_entity_detection", False):
    ...
    async def _try_edit_rich(
        ...
        """Attempt final editMessageText with official Bot API ``rich_message``.

        Returns SendResult on rich success/transient failure, or None when the
        caller should fall back to the legacy MarkdownV2 edit path.
        """
        payload: Dict[str, Any] = {
            "chat_id": int(chat_id),
        ...
        try:
            ...
            if self._is_rich_fallback_error(exc):
                if self._is_rich_capability_error(exc):
                    self._rich_send_disabled = True
                logger.debug(
                    "[%s] editMessageText rich_message rejected (%s) — falling back to MarkdownV2",
                    self.name, exc,
                )
                return None
        ...
        if isinstance(msg, dict):
            returned_message_id = msg.get("message_id")
            if returned_message_id is None:
                returned_message_id = (msg.get("result") or {}).get("message_id")
        else:
            returned_message_id = getattr(msg, "message_id", None)
        return SendResult(
            success=True,
            message_id=str(returned_message_id) if returned_message_id is not None else message_id,
        )
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            webhook_url = self._telegram_env_value("WEBHOOK_URL", "").strip()
            ...
            if webhook_url:
                ...
                webhook_port = int(self._telegram_env_value("WEBHOOK_PORT", "8443"))
                webhook_secret = self._telegram_env_value("WEBHOOK_SECRET", "").strip()
    ...
    async def edit_message(
        ...
        *,
        ...
        # Official Bot API rich messages support up to 32,768 UTF-8 characters
        # and editMessageText accepts rich_message=InputRichMessage. Try that
        # before the legacy 4,096-codeunit MarkdownV2 split so final streamed
        # replies keep tables, task lists, headings, details and formulas intact.
        if finalize and self._should_attempt_rich_edit(content, metadata=metadata):
            rich_result = await self._try_edit_rich(chat_id, message_id, content, metadata)
            if rich_result is not None:
                return rich_result

        # Pre-flight: if content already exceeds the legacy text limit, split
        # and deliver without round-tripping a doomed MarkdownV2 edit.
