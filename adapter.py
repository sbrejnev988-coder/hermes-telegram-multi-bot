...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, platform: "Platform" = None):
        if platform is None:
            # Keep default for backward compat with plugin registry
...
def _build_adapter(config, platform_name: str = "telegram"):
    ...
    The *platform_name* parameter is used for multi-Telegram slots (telegram2,…)."""
    from gateway.config import Platform
    plat = Platform(platform_name) if platform_name != "telegram" else Platform.TELEGRAM
    adapter = TelegramAdapter(config, platform=plat)
    try:
...
def register(ctx) -> None:
    ...
    # Register primary Telegram slot
    ...
    # ── Register secondary Telegram slots (telegram2 … telegram20) ──
    for slot_num in range(2, 21):
        slot_name = f"telegram{slot_num}"
        prefix = f"TELEGRAM{slot_num}"
        ctx.register_platform(
            name=slot_name,
            label=f"Telegram {slot_num}",
            adapter_factory=lambda config, sn=slot_name: _build_adapter(config, platform_name=sn),
            check_fn=check_telegram_requirements,
            is_connected=_is_connected,
            required_env=[f"{prefix}_BOT_TOKEN"],
            install_hint="pip install 'hermes-agent[telegram]'",
            apply_yaml_config_fn=_apply_yaml_config,
            allowed_users_env=f"{prefix}_ALLOWED_USERS",
            allow_all_env=f"{prefix}_ALLOW_ALL_USERS",
            cron_deliver_env_var=f"{prefix}_HOME_CHANNEL",
            standalone_sender_fn=_standalone_send,
            max_message_length=4096,
            emoji="✈️",
            allow_update_command=True,
