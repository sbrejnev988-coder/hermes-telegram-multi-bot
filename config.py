...
# ── Multi-Telegram slot support ──────────────────────────────────────────
# Hermes supports up to 20 Telegram bot slots: telegram, telegram2, … telegram20.
# The primary slot uses the legacy env var names (TELEGRAM_BOT_TOKEN, etc.).
# Secondary slots use TELEGRAM2_BOT_TOKEN … TELEGRAM20_BOT_TOKEN.
_TELEGRAM_MAX_SLOTS = 20
_TELEGRAM_SLOT_PREFIX = "telegram"


def is_telegram_platform(platform: Any) -> bool:
    """Return True for any Telegram platform slot (telegram, telegram2, …)."""
    if hasattr(platform, "value"):
        platform = platform.value
    return isinstance(platform, str) and platform.startswith(_TELEGRAM_SLOT_PREFIX)


def telegram_env_prefix(platform: Any) -> str:
    """Return the env-var prefix for a Telegram platform slot.

    Primary slot (``telegram``) returns ``TELEGRAM`` (legacy).
    Secondary slots return ``TELEGRAM2`` … ``TELEGRAM20``.
    """
    if hasattr(platform, "value"):
        platform = platform.value
    platform = str(platform).lower()
    if platform == _TELEGRAM_SLOT_PREFIX:
        return "TELEGRAM"
    return platform.upper().replace("-", "_")


def _telegram_slot_env(name: str, slot: int) -> str:
    """Build env-var name for a given Telegram slot and config key.

    Slot 1 (primary) uses the legacy ``TELEGRAM_{name}`` form.
    Slots 2–20 use ``TELEGRAM{slot}_{name}``.
    """
    if slot == 1:
        return f"TELEGRAM_{name}"
    return f"TELEGRAM{slot}_{name}"


def _telegram_slot_list() -> list:
    """Return list of (slot_number, slot_name) tuples for all supported Telegram slots."""
    slots = [(1, _TELEGRAM_SLOT_PREFIX)]
    for n in range(2, _TELEGRAM_MAX_SLOTS + 1):
        slots.append((n, f"{_TELEGRAM_SLOT_PREFIX}{n}"))
    return slots
...
class Platform(Enum):
    ...
    def _missing_(cls, value):
        """Accept unknown platform names for known plugin adapters and Telegram slots.

        Creates a pseudo-member cached in ``_value2member_map_`` so that
        ``Platform("irc") is Platform("irc")`` holds True (identity-stable).
        Telegram slots (telegram2 … telegram20) are always accepted.
        ...
        # ── Multi-Telegram slots: accept telegram2 … telegram20 ──
        if re.match(r'^telegram\d+$', value) and value != "telegram":
            slot_num = int(value[len("telegram"):])
            if 2 <= slot_num <= _TELEGRAM_MAX_SLOTS:
...
def _validate_gateway_config(config: "GatewayConfig") -> None:
    ...
    # Add all Telegram slot token env names (telegram2 … telegram20)
    for _slot_num, _slot_name in _telegram_slot_list():
        if _slot_num >= 2:
            _plat = Platform(_slot_name)
            if _plat is not None:
                _token_env_names[_plat] = f"TELEGRAM{_slot_num}_BOT_TOKEN"
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    # ── Multi-Telegram slots (telegram2 … telegram20) ──
    for _slot_num, _slot_name in _telegram_slot_list():
        if _slot_num == 1:
        ...
        if _plat is None:
            continue
        _prefix = f"TELEGRAM{_slot_num}"
        _token = os.getenv(f"{_prefix}_BOT_TOKEN")
        if _token:
            _tg_cfg = _enable_from_env(_plat)
            _tg_cfg.token = _token
        # Home channel
        _home = os.getenv(f"{_prefix}_HOME_CHANNEL")
        if _home and _plat in config.platforms:
            config.platforms[_plat].home_channel = HomeChannel(
                platform=_plat,
                chat_id=_home,
                name=os.getenv(f"{_prefix}_HOME_CHANNEL_NAME", "Home"),
                thread_id=os.getenv(f"{_prefix}_HOME_CHANNEL_THREAD_ID") or None,
            )
        # Reply mode
        _reply = os.getenv(f"{_prefix}_REPLY_TO_MODE", "").lower()
        if _reply in {"off", "first", "all"}:
            if _plat not in config.platforms:
                config.platforms[_plat] = PlatformConfig()
            config.platforms[_plat].reply_to_mode = _reply
        # Allowed users
        _allowed = os.getenv(f"{_prefix}_ALLOWED_USERS")
        if _allowed is not None:
            if _plat not in config.platforms:
                config.platforms[_plat] = PlatformConfig()
            config.platforms[_plat].extra["allowed_users"] = _allowed
        # Allowed chats
        _allowed_chats = os.getenv(f"{_prefix}_ALLOWED_CHATS")
        if _allowed_chats is not None:
            if _plat not in config.platforms:
                config.platforms[_plat] = PlatformConfig()
            config.platforms[_plat].extra["allowed_chats"] = _allowed_chats
